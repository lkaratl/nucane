...
use domain_model::{Action, CreateSimulation, Currency, CurrencyPair, Exchange, InstrumentId, MarketType, NewDeployment, Order, OrderActionType, OrderMarketType, OrderStatus, OrderType, Position, Side, Simulation, SimulationDeployment, SimulationPosition, Size, Tick};
...
impl<E: EngineApi, S: StorageApi, I: InteractorApi> Simulator<E, S, I> {
    ...
    async fn create_deployments(&self, simulation: &mut Simulation) {
        let create_deployments: Vec<_> = simulation.deployments.iter()
            .map(|strategy| convert_to_create_deployment_dto(strategy.clone(), simulation.id))
            .collect();
        let created_deployments = self.engine_client.deploy(&create_deployments)
            .await
            .unwrap();
    ...
    async fn delete_deployments(&self, deployments: &[SimulationDeployment]) {
        for deployment in deployments {
            if let Some(deployment_id) = deployment.deployment_id {
                self.engine_client.delete_deployment(deployment_id)
                    .await
                    .unwrap();
            }
        }
    ...
    async fn execute_order(&self, order: &mut Order, quote: f64, positions: &mut Vec<SimulationPosition>, logger: &mut Logger) {
        ...
        match order.side {
            Side::Buy => {
                self.update_positions(target_size, source_size, fee_percent, target_position, source_position, logger).await;
            }
            Side::Sell => {
                self.update_positions(source_size, target_size, fee_percent, source_position, target_position, logger).await;
            }
        }
    ...
    async fn get_ticks(&self, logger: &mut Logger, simulation: &Simulation, from: DateTime<Utc>, to: DateTime<Utc>) -> Vec<Tick> {
        ...
        for deployments in &simulation.deployments {
            ...
            for instrument_id in &deployments.subscriptions {
                let sync_report = self.storage_client.sync(instrument_id, &[timeframe], from, Some(to)).await.unwrap();
    ...
    async fn update_positions(&self, target_size: f64, source_size: f64, fee_percent: f64, target_position: Option<&mut SimulationPosition>, source_position: Option<&mut SimulationPosition>, logger: &mut Logger) {
        let source_position = source_position.expect("No source asset to execute order");
        source_position.end -= source_size;
        let _ = self.storage_client.save_position(Position::from(source_position.clone())).await;
        ...
        let _ = self.storage_client.save_position(Position::from(target_position.clone())).await;
...
fn convert_to_create_deployment_dto(value: SimulationDeployment, simulation_id: Uuid) -> NewDeployment {
    NewDeployment {
