...
impl SimulationService {
    pub fn new(strategy_engine_client: StrategyEngineClient,
               storage_client: StorageClient,
    ) -> Self {
        SimulationService {
            strategy_engine_client,
            storage_client,
        }
    }
    ...
    pub async fn run(&self, simulation: Simulation) -> SimulationReport {
        ...
        let deployment = self.strategy_engine_client.create_deployment(
            Some(simulation.id),
        ...
        while batch_end != simulation.end {
            ...
            for tick in &ticks {
                ...
                let actions = self.strategy_engine_client.create_actions(tick).await.unwrap();
        ...
        self.strategy_engine_client.remove_deployment(deployment.id)
            .await
            .unwrap();
