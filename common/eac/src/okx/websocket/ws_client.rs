...
impl OkxWsClient {
    ...
    pub async fn business<H: WsMessageHandler>(
        demo: bool,
        ws_public_url: &str,
        handler: H,
    ) -> Self {
        ...
        if demo {
            ws_public_url = "wss://wspap.okx.com:8443/ws/v5/business?brokerId=9999".to_string();
            // todo unhardcode host
        }
        Self::new(&ws_public_url, None, handler).await
    }
                    ...
                    Ok(message) => match message {
                        WsMessage::Text(message) => {
                            ...
                            let message: Message = serde_json::from_str(&message.clone())
                                .expect("Cannot deserialize message from OKX");
